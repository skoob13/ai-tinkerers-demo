...
async function fetchPostHogKey() {
    ...
    try {
        ...
        const result = await client.query(
            ...
            process.env.POSTHOG_TEAM_ID ? [process.env.POSTHOG_TEAM_ID] : []
        )

        if (result.rows.length === 0) {
            console.warn(
                `⚠ ${process.env.POSTHOG_TEAM_ID ? `No team found with ID ${process.env.POSTHOG_TEAM_ID}` : 'No team found'}`
            )
    ...
    if (fs.existsSync(envPath) && !process.env.FORCE_FETCH_KEY) {
        ...
        if (keyMatch && keyMatch[1] && keyMatch[1].trim() && keyMatch[2].trim() === teamCreatedAt.toISOString()) {
            ...
            console.info('  `rm .env.local` or `export FORCE_FETCH_KEY=1` to fetch anew\n')
            return
        }
    }
